	...
	while (!q.empty())
	{
		...
		for (ll i=0;i<4;i++)
		{
			...
			if (newx>=0 && newx<n && newy>=0 && newy<m && grid[newx][newy]!='#' && !vis[newx][newy])
			{
				q.push({newx,newy});
				vis[newx][newy]=true;
				if (i==0) dir[newx][newy]='R';
				else if (i==1) dir[newx][newy]='D';
				else if (i==2) dir[newx][newy]='L';
				else dir[newx][newy]='U';
			}
		}
	}
	if (!vis[X][Y]) { NN; return; }
	cout<<"YES"<<endl;
	string path;
	while (!(X==x && Y==y))
	{
