...
// Memory Limit: 
// Time Limit: 
// Problem tag: dfs, cycle detection
// TC: 
// SC: 
...
bool isCycle(ll node, ll par, vll adj[], vbool &vis)
{
	vis[node]=true;
	for (auto &it: adj[node])
	{
		...
		else if (it!=par) { path.pb(it); return true; }
	}
	return false;
}
...
void solve()
{
	...
	for (ll i=1;i<=n;i++)
	{
		if (!vis[i])
		{
			...
			{
				ll ind=0;
				while (path[ind]!=path.back()) ind++;
				cout<<path.size()-ind<<endl;
