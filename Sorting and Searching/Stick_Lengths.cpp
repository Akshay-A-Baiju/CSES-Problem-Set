...
void solve()
{
    ll n; cin>>n;
    vll p(n); cin>>p;
    // sort the sticks and construct a prefix and suffix array to find the min cost to make all stick length to current stick length
    // can be optimized by removing prefix array as well
    sort(all(p));
    vll suffix(n);
    suffix[n-1]=0;
    for (ll i=n-2;i>=0;i--)
    {
        ll elemsToRight=n-i-1;
        ll cost=(p[i+1]-p[i])*elemsToRight;
        suffix[i]=suffix[i+1]+cost;
    }
    ll addCost=0,ans=inf;
    for (ll i=0;i<n;i++)
    {
        // try making all stick length to p[i]
        ll elemsToLeft=i;
        ll cost=elemsToLeft?((p[i]-p[i-1])*elemsToLeft):0LL;
        addCost+=cost;
        ll totalCost=addCost+suffix[i];
