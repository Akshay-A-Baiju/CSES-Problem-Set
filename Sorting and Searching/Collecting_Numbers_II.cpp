...
void solve()
{
    ll n,m; cin>>n>>m;
    vll v(n); cin>>v;
    vll pos(n+1),getVal(n);
    for (ll i=0;i<n;i++) pos[v[i]]=i, getVal[i]=v[i];
    ll ans=1;
    for (ll i=2;i<=n;i++)
    {
    ...
    {
        ll i,j; cin>>i>>j; i--; j--;
        ll x=getVal[i],y=getVal[j];
        ll prev_x_pos=i;
        ll prev_y_pos=j;
        ll prev_x_minus_1_pos=(x-1>=1)?pos[x-1]:0;
        ll prev_x_plus_1_pos=(x+1<=n)?pos[x+1]:0;
        ll prev_y_minus_1_pos=(y-1>=1)?pos[y-1]:0;
        ll prev_y_plus_1_pos=(y+1<=n)?pos[y+1]:0;
        pos[x]=j;
        pos[y]=i;
        getVal[i]=y;
        getVal[j]=x;
        map <pll,ll> mp;
        // check x with x-1 pos
        if (x-1>=1)
        {
            if (prev_x_pos>prev_x_minus_1_pos && pos[x]<pos[x-1]) mp[{x-1,x}]++;
            else if (prev_x_pos<prev_x_minus_1_pos && pos[x]>pos[x-1]) mp[{x-1,x}]--;
        }
        // check x with x+1 pos
        if (x+1<=n)
        {
            if (prev_x_pos<prev_x_plus_1_pos && pos[x]>pos[x+1]) mp[{x,x+1}]++;
            else if (prev_x_pos>prev_x_plus_1_pos && pos[x]<pos[x+1]) mp[{x,x+1}]--;
        }
        // check y with y-1 pos
        if (y-1>=1)
        {
            if (prev_y_pos>prev_y_minus_1_pos && pos[y]<pos[y-1]) mp[{y-1,y}]++;
            else if (prev_y_pos<prev_y_minus_1_pos && pos[y]>pos[y-1]) mp[{y-1,y}]--;
        }
        // check y with y+1 pos
        if (y+1<=n)
        {
            if (prev_y_pos<prev_y_plus_1_pos && pos[y]>pos[y+1]) mp[{y,y+1}]++;
            else if (prev_y_pos>prev_y_plus_1_pos && pos[y]<pos[y+1]) mp[{y,y+1}]--;
        }
        ll change=0;
        for (auto &pr: mp)
        {
