...
void solve()
{
    ll x,n; cin>>x>>n;
    vll p(n); cin>>p;
    set <pll> passage;    // { length, start index}
    set <pll> index;      // { start index, length}
    passage.insert({x,0LL});
    index.insert({0LL,x});
    for (auto &light: p)
    {
        auto it=index.lower_bound({light,0LL});
        --it;
        ll ind=it->ff,length=it->ss;
        passage.erase({length,ind});
        index.erase({ind,length});
        ll length1=light-ind;
        ll length2=length-length1;
        passage.insert({length1,ind});
        index.insert({ind,length1});
        passage.insert({length2,light});
        index.insert({light,length2});
        ll ans=(--passage.end())->ff;
        cout<<ans<<" ";
    }
}
