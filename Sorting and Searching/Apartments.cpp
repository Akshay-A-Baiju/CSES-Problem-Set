...
void solve()
{
    ...
    for (auto &it: b) apartments.insert(it);
    ll ans=0;
    for (auto &it: a)
    {
        ll min_size=it-k,max_size=it+k;
        auto itt=apartments.lower_bound(min_size);
        if (itt!=apartments.end() && *itt<=max_size)
        {
            ans++;
            apartments.erase(itt);
        }
    }
    cout<<ans;
}
