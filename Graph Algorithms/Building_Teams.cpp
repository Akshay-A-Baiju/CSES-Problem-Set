...
// ********     ACCEPTED     ********
bool isBipartite(ll node, ll tm, vll adj[], vll &team)
{
    team[node]=tm;
    for (auto &it: adj[node])
    {
        if (team[it]==-1)
        {
            if (!isBipartite(it,1-team[node],adj,team)) return false;
        }
...
void solve()
{
    ...
    for (ll i=0;i<n;i++)
    {
        if (team[i]==-1)
        {
            if (!isBipartite(i,0,adj,team)) { cout<<"IMPOSSIBLE"; return; }
        }
    }
    for (auto &it: team) cout<<it+1<<" ";
}
