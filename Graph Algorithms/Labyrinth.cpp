...
// ********     ACCEPTED     ********
pll getPosition(vector <string> &grid, char targ)
{
    ...
    for (ll i=0;i<n;i++)
    {
        for (ll j=0;j<m;j++)
        {
            if (grid[i][j]==targ) return make_pair(i,j);
        }
    }
    return make_pair(-1,-1);
}

void bfs(ll X, ll Y, vector <string> &grid, vvll &minDist)
{
    ll n=sz(grid),m=sz(grid[0]);
    queue <pll> q;
    q.push({X,Y});
    ...
    while (!q.empty())
    {
        ...
        for (ll k=0;k<4;k++)
        {
            ...
            if (grid[x][y]!='#' && minDist[x][y]==inf)
            {
                q.push({x,y});
                minDist[x][y]=1+minDist[i][j];
            }
        }
    }
}
...
void solve()
{
    ...
    pll A=getPosition(grid,'A');
    pll B=getPosition(grid,'B');
    vector <vbool> vis(n,vbool (m,false));
    vvll minDist(n, vll (m,inf));
    bfs(B.ff,B.ss,grid,minDist);
    if (minDist[A.ff][A.ss]==inf) NN;
    else
    {
        YY; cout<<endl;
        string path="";
        getPath(A.ff,A.ss,B.ff,B.ss,minDist,path);
