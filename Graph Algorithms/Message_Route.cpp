...
// ********     ACCEPTED     ********
void bfs(ll s, vll adj[], vll &minDist)
{
    queue <ll> q;
    q.push(s);
    minDist[s]=0;
    while (!q.empty())
    {
        ...
        for (auto &it: adj[node])
        {
            ...
            {
                q.push(it);
                minDist[it]=1+minDist[node];
            }
        }
    }
}

void getPath(ll node, ll tar, vll adj[], vll &minDist, vll &path)
{
    ...
    for (auto &it: adj[node])
    {
        ...
        {
            getPath(it,tar,adj,minDist,path);
            break;
        }
    }
}
...
void solve()
{
    ...
    vll minDist(n,inf);
    bfs(n-1,adj,minDist);
    if (minDist[0]==inf) cout<<"IMPOSSIBLE";
    else
    {
        vll path;
        getPath(0,n-1,adj,minDist,path);
        cout<<sz(path)<<endl<<path;
    }
}
