...
bool isCyclic(ll node, ll par, vll adj[], vbool &vis, vll &path)
{
    vis[node]=true;
    path.push_back(node);
    for (auto &it: adj[node])
    {
        if (!vis[it])
        {
            if (isCyclic(it,node,adj,vis,path)) return true;
        }
        else if (it!=par)
        {
            cyclic_path=path;
            cyclic_path.push_back(it);
            path.pop_back();
            return true;
        }
    }
...
void solve()
{
    ...
    for (ll i=0;i<n;i++)
    {
        ...
        {
            vll path;
            if (isCyclic(i,-1,adj,vis,path))
            {
                ll j=0;
                while (j<sz(cyclic_path) && cyclic_path[j]!=cyclic_path.back()) j++;
                cout<<sz(cyclic_path)-j<<endl;
                while (j<sz(cyclic_path)) cout<<cyclic_path[j++]+1<<" ";
                return;
            }
        }
    }
