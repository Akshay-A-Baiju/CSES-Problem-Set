...
void fun(ll node, ll par, vll adj[])	// fun(i,p,0) -> dont take edge with node i, fun(i,p,1) -> take edge with node i
{
	bool isLeaf=true;
	...
	ll sz=prefix.size();
	for (ll i=1;i<sz;i++) prefix[i]+=prefix[i-1];
	for (ll i=sz-2;i>=0;i--) suffix[i]+=suffix[i+1];
	// dont take edge with node
	dp[node][0]=suffix[0];
	// take edge with node
	ll ind=0;
	for (auto &it: adj[node])
	{
		if (it==par) continue;
		ll left=(ind==0)?0:prefix[ind-1];
		ll right=(ind==sz-1)?0:suffix[ind+1];
		dp[node][1]=max(dp[node][1],1+left+right+dp[it][0]);
		ind++;
	}
}
