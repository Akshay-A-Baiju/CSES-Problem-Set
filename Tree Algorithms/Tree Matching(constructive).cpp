...
void solve()
{
	...
	while (!leaf.empty())
	{
		...
		{
			adj[it].erase(next);
			if (adj[it].size()==1) leaf.ins(it);
			if (adj[it].size()==0 && leaf.find(it)!=leaf.end()) leaf.erase(it);
		}
		adj[next].clear();
		ans++;
		if (leaf.find(next)!=leaf.end()) leaf.erase(next);
	}
	cout<<ans;
}
